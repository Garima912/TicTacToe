...
public class MinMax {
	...
	private ArrayList<Node> stateList;	//a list to contain all possible moves for a player
	...
	MinMax(String[] init, int player)
	{
		...
		if(player == 1){
			...
			stateList = createStateList("min", initNode);		//get list of all O moves from initial state
		}
		...
		setStateList_MinMaxValues(stateList, player);			//set initial state minMax values for X moves
	 ...
	 * @return
	 */
	ArrayList<Node> findMoves(int player)
	{
